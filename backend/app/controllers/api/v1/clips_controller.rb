class Api::V1::ClipsController < Api::V1::BaseController
  ...
  def index
    ...
    if params[:favorites] == "true"
      clips = clips.where(is_favorite: true)
    end
  ...
  def create
    ...
    detection = ClipDetectorService.detect(
      clip_params[:content]
    )
  ...
  def bulk_update
    clips = current_user.clips.where(
      id: params[:clip_ids]
    )
  ...
  def related
    ...
    related_clips =
      current_user.clips
        ...
        same_language =
          item.language == clip.language

        same_collection =
          item.collection_id == clip.collection_id
    ...
    render json: related_clips.first(5)
  end
